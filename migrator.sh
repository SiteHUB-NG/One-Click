...
# available fo DD where * ONE-CLICK MIGRATOR MODE * reinstall by ~bin456789 to #
...
before_migration() {
  local local_disk_name=$(findmnt -n -o SOURCE /)
  ...
  # === Prepare Root Lookup ====
  e2label "$local_disk_name" CLONE_ROOT || tune2fs -L CLONE_ROOT "$local_disk_name"
  cp /etc/fstab /etc/fstab.one_click_bak
  cp /boot/grub/grub.cfg /boot/grub/grub.cfg.one_click_bak
  # ==== Prepare GRUB ====
  update-grub
  sed -Ei 's,^UUID=[^ ]* /,LABEL=CLONE_ROOT /,' /etc/fstab
  sed -Ei '
    /GRUB_DISABLE_LINUX_UUID/ ! { 
      $p;
      s/.*/GRUB_DISABLE_LINUX_UUID=true/
    };
    /GRUB_DISABLE_LINUX_UUID/ {
      s/^#?(GRUB_DISABLE_LINUX_UUID=).*/\1true/
    }
  ' /etc/default/grub
  sed -Ei.one-click-backup '
    ...
    s/(GRUB_TIMEOUT=)"?[^"]*"?/\15/
  ' /etc/default/grub
  sed -i 's/search --no-floppy --fs-uuid --set=root .*/search --no-floppy --label CLONE_ROOT --set=root/' /boot/grub/grub.cfg
...
menuentry "Recovery Environment (SSH)" {
    ...
    insmod ext2
    insmod search_label
    search --no-floppy --label --set=root CLONE_ROOT
    linux /boot/recovery/vmlinuz-${kernel_version} root=/dev/ram0 rw rdinit=/init console=tty0 console=ttyS0,115200n8
...
EOF
  ...
  cp -f "/boot/vmlinuz-$kernel_version" "$recovery_dir/vmlinuz-$kernel_version"
  # ==== Copy kernel ====
...
EOF
  # ==== Recory Script ====
  cat > "$recovery_dir/sbin/remote_fix.sh" <<'EOF'
#!/bin/sh
echo "--- AUTOMATED POST-MIGRATION REPAIR ---"
mkdir -p /mnt
tune2fs -L CLONE_ROOT /dev/sda1 || e2label /dev/sda1 CLONE_ROOT
mount -t ext4 /dev/disk/by-label/CLONE_ROOT /mnt || mount /dev/sda1 /mnt
if [ $? -eq 0 ]; then
  echo "Migrated Root Detected and Mounted."
  if [ -f /mnt/etc/os-release ]; then
    dst_id=$(grep -E '^ID=' /mnt/etc/os-release | cut -d= -f2 | tr -d '"')
  fi
  for i in dev proc sys run; do
    mount --bind /$i /mnt/$i
  done
  dst_disk=$(lsblk -no PKNAME $(readlink -f /dev/disk/by-label/CLONE_ROOT) | head -n1)
  if [ -z "$dst_disk" ]; then
    dst_disk="sda"
  fi
  case "$dst_id" in
    ubuntu|debian|kali|raspbian) expected_grub="grub"                          ;;
    centos|rhel|fedora|rocky|almalinux) expected_grub="grub2"                  ;;
    *) [ -d /mnt/boot/grub2 ] && expected_grub="grub2" || expected_grub="grub" ;;
  esac
  if [ ! -d "/mnt/boot/$expected_grub" ]; then
    if [ "$expected_grub" = "grub2" ]; then
      ln -sf /boot/grub /mnt/boot/grub2
    else
      ln -sf /boot/grub2 /mnt/boot/grub
    fi
  fi
  chroot /mnt /usr/bin/rm -f /boot/grub/custom.cfg
  chroot /mnt grub-install --recheck "/dev/$dst_disk"
  if chroot /mnt command -v update-grub >/dev/null 2>&1; then
    chroot /mnt update-grub
  elif chroot /mnt command -v grub2-mkconfig >/dev/null 2>&1; then
    chroot /mnt grub2-mkconfig -o /boot/$expected_grub/grub.cfg
  fi
  sync
...
EOF
  chmod +x "$recovery_dir/sbin/remote_fix.sh"
...
for mod in virtio_blk virtio_pci virtio_scsi nvme ahci sd_mod; do
...
if [ -b /dev/disk/by-label/CLONE_ROOT ]; then
  mkdir -p /mnt
  mount /dev/disk/by-label/CLONE_ROOT /mnt
...
after_migration() {
  # ==== Clean Local System ==== 
  [[ -f /etc/fstab.one_click_bak ]] && mv -f /etc/fstab.one_click_bak /etc/fstab
  info "fstab restored"
  [[ -f /etc/default/grub.one-click-backup ]] && mv -f /etc/default/grub.one-click-backup /etc/default/grub
  info "/etc/default/grub restored"
  [[ -f /boot/grub/grub.cfg.one_click_bak ]] && mv -f /boot/grub/grub.cfg.one_click_bak /boot/grub/grub.cfg
  info "/boot/grub/grub.cfg restored"
  rm -f /boot/grub/custom.cfg
  update-grub
  rm -rf /boot/recovery
  success "System GRUB restored."
}
...
destination_finalize() {
  info "Starting Post-Migration Repair"
  mount -o remount,rw /mnt 2>/dev/null || mount -o remount,rw /
  rm -f /boot/grub/custom.cfg
  rm -rf /boot/recovery
  ...
  success "Migration Finalized. Rebooting into Production..."
  sync
  reboot
}
  ...
  while true; do
    ...
    case "$req" in
      ...
      key|ssh|ssh_key)
        ...
        ;;
      *)
        warn "Invalid selection: '${red}$req${reset}'" "Please choose 'password' or 'key'."
        # No 'return' or 'break' here means it loops back to the question
        ;;
    esac
  done
}
...
dd_migrate() {
  ...
  trap "after_migration; $current_trap" EXIT
...
EOF
  ...
  cat << EOF > "$net_config"
network_card=$(sed -En '/2: ([^:]*):.*/s//\1/p' <(ip a))
ids=(${ids[@]})
for id in ${ids[@]} ; do
  case \${id:-} in
    debian|ubuntu)
      ...
      # ==== Force GRUB to use a standard serial and terminal console ====
      sed -i 's/GRUB_CMDLINE_LINUX_DEFAULT=".*/GRUB_CMDLINE_LINUX_DEFAULT="console=tty0 console=ttyS0,115200n8"/' /etc/default/grub
      update-grub
      sed -i.one-click-backup "s/eth0/\$network_card/g" /etc/network/interfaces
      ip addr flush dev "\$network_card" || true
      ifdown "\$network_card" &> /dev/null || true
      ifup "\$network_card" &> /dev/null || true
      ;;
    rhel|centos|rocky|almalinux|fedora)
      ...
      if command -v nmcli &> /dev/null; then
        ...
        nmcli device set "\$network_card" managed yes
        nmcli connection reload
        nmcli device disconnect \$network_card
        nmcli device connect \$network_card
        ...
        sed -i 's/GRUB_CMDLINE_LINUX_DEFAULT=".*/GRUB_CMDLINE_LINUX_DEFAULT="console=tty0 console=ttyS0,115200n8"/' /etc/default/grub
        update-grub
        sed -i.one-click-backup "s/eth0/\$network_card/g" /etc/sysconfig/network-scripts/ifcfg-\$network_card
        systemctl restart network || systemctl restart NetworkManager
      fi
      ;;
  esac
...
EOF
  ...
  sleep 0.3
  if ! sshpass -p "$pass" ssh -T -o ServerAliveInterval=30 -o ServerAliveCountMax=10 -o StrictHostKeyChecking=no "${user}@${destination_server}" "exit" 2> /dev/null ; then
    error " " "${red}SSH connection failed!${reset}" \
