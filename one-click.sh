...
if [[ "$#" -eq 0 || "${1:-}" == "-h" || "${1:-}" == "--help" || "${1:-}" == "help" ]]; then
  printf '%s\n' \
    ...
    "      (add-to|append)     Add additional IPs mapped to aliases to extend batch processing functionality. Key alias must already exist else use the remember command first.n
